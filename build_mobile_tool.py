...
function methodsHtml(){
  ...
  const funnel=(rows.dlt||[]).find(x=>x.method==='funnel');
  if(funnel&&funnel.exclusions){
    const fmtDrops=(arr)=>arr&&arr.length?arr.map(n2).join(' '):'无';
    h+='<div class="card"><h2>我的分层分区方法排除明细</h2>';
    (funnel.exclusions.main||[]).forEach(item=>{const parts=item.regions.map((r,i)=>'第'+(i+1)+'区去掉 '+fmtDrops(r));h+='<p class="note">第'+item.layer+'层：'+parts.join('；')+'</p>';});
    (funnel.exclusions.back||[]).forEach(item=>{h+='<p class="note">后区第'+item.layer+'层：去掉 '+fmtDrops(item.regions[0])+'</p>';});
    h+='</div>';
  }
